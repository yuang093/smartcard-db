...
- [ ] 實作「批次操作 (Batch Actions)」，支援多選並進行批次刪除與批次上標籤。

4.4 行動裝置優化 (Mobile UX)
- [ ] 手機版 UI 調整（確保按鈕大小、排版、彈窗在手機上顯示正常）。

## Phase 5: 部署、測試與維運 (DevOps)
- [x] 配置反向代理，綁定網域 card.yuang093.cc 並申請 SSL 憑證 (Cloudflare Tunnel)。
- [ ] 手機端實機測試 UI/UX 順暢度。
- [ ] 撰寫自動化備份腳本 (Shell Script) 備份 PostgreSQL 與 volumes/uploads/ 圖檔，並可設定 Cron 定期執行。

## Phase 6: 系統管理與監控 (Admin Setup)
- [ ] 實作 Admin 專屬 /setup 頁面 (RBAC: 僅 admin 帳號登入首頁顯示入口，按鈕放標籤管理左側並用不同顏色區分)。
- [ ] 實作帳戶管理：可刪除帳戶、新增帳號、修改密碼。
- [ ] 實作儲存空間儀表板：顯示系統總名片數、圖檔佔用的硬碟空間 (MB/GB)。
